...
elastix_path = os.path.join(os.getcwd(), 'elastix-5.2.0-mac', 'bin', 'elastix')
flirt_path = os.path.join(os.getcwd(), 'fsl', 'bin', 'flirt')

print('The location of MNI152 bone:' , MNI_152_bone)
print('The location of MNI152:' , MNI_152)
...
call([flirt_path, '-in', ct_scan_wodevice_bone, '-ref', MNI_152_bone, '-omat', nameOfAffineMatrix, '-bins', '256', '-searchrx', '-180', '180', '-searchry', '-180', '180', '-searchrz', '-180', '180', '-dof', '12', '-interp', 'trilinear'])

call([flirt_path, '-in', ct_scan_wodevice, '-ref', MNI_152, '-applyxfm', '-init', nameOfAffineMatrix, '-out', ct_scan_wodevice[:ct_scan_wodevice.find('.nii.gz')]+'_MNI152.nii.gz'])
...
call([flirt_path, '-in', ct_scan_wodevice_contraststretching, '-ref', MNI_152, '-applyxfm', '-init', nameOfAffineMatrix, '-out', ct_scan_wodevice_contraststretching[:ct_scan_wodevice_contraststretching.find('.nii.gz')]+'_MNI152.nii.gz'])

#call([elastix_path, '-m', ct_scan_wodevice_contraststretching[:ct_scan_wodevice_contraststretching.find('.nii.gz')]+'_MNI152.nii.gz', '-f', MNI_152, '-out', os.path.dirname(ct_scan_path), '-p', bspline_path])


fixed_image = sitk.ReadImage(MNI_152)
moving_image = sitk.ReadImage(ct_scan_wodevice_contraststretching[:ct_scan_wodevice_contraststretching.find('.nii.gz')]+'.nii.gz')
...
print("Output File Name: " + str(name_ct_with_MNI))
